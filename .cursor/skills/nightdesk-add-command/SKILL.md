...
3. **Adaptador** — `#[tauri::command]` en `src-tauri/src/commands.rs`. Primero `crate::auth::require(&state, session_token.as_deref(), admin)?;`, después `conn(&state)`, después `service::…`. Impresión solo aquí. **Mutación operativa nueva → `wake_push(&state)`** después de soltar el mutex de SQLite, para que el worker I07 drene la cola.
