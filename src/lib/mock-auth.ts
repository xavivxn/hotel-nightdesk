...
function fail(code: string, text: string): never { throw `${code}: ${text}`; }
...
export async function mockAuth(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (name === "auth_setup" || name === "auth_create_user") {
    if (name === "auth_setup") {
      ...
      if (previous && previous !== `mock:${args.legacy_pin ?? ""}`) throw "Ingresá el PIN anterior para configurar el administrador";
    } else requireMockSession(args, true);
    const payload = args.payload as CreateUserPayload;
    const username = payload.username.trim().toLowerCase();
    if (!username || username.length > 64) throw "Ingresá un usuario de 1 a 64 caracteres";
    const length = new TextEncoder().encode(payload.password).length;
    if (length < 1 || length > 128) throw "La contraseña debe tener entre 1 y 128 bytes";
    const role = name === "auth_setup" ? "admin" : payload.role;
    if (role !== "admin" && role !== "recepcion") throw "Rol inválido";
    if (db.users.some(u => u.username === username)) throw "Ese usuario ya existe";
    ...
    if (latest.users.some(u => u.username === username)) throw "Ese usuario ya existe";
  ...
  throw "Comando de acceso no implementado";
}
