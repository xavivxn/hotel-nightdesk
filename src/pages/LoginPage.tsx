...
export function LoginPage({
  ...
  return <div className="grid h-full min-h-0 place-items-center overflow-auto p-6"><form className="card w-full max-w-md space-y-4 rounded-lg p-8" onSubmit={async e => {
    ...
    <p className="text-sm text-[var(--muted)]">{remote
      ...
      : needsSetup
        ...
        : "Ingresá con tu cuenta individual. Funciona sin internet."}</p>
    {notice && <p role="status" className="text-sm text-[var(--muted)]">{notice}</p>}
    <label className="block text-sm">{remote ? "Email" : "Usuario"}<Input autoFocus required maxLength={64} autoComplete={remote ? "username" : "username"} value={username} onChange={e => setUsername(e.target.value)} /></label>
